...
/* explicit */
RenderData::RenderData(TileNode* node, bool secTileActive)
    : mHasBounds(false)
    , mNode(node)
    , mTexLayer(-1)
    , mLastFrame(-1),
    mSecTileActive(secTileActive) {
}
...
void RenderData::setNode(TileNode* node, bool secTileActive) {
  mNode = node;
  mSecTileActive = secTileActive;
}
...
bool RenderData::getSecTileActive() {
  return mSecTileActive;
}
