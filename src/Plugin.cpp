...
void Plugin::init() {
  ...
  mPluginSettings->mMapCache.connect([this](std::string const& val) {
    for (auto&& body : mLodBodies) {
      auto src =
          std::dynamic_pointer_cast<TileSourceWebMapService>(body.second->getDEMtileSources()[0]);
      if (src) {
        src->setCacheDirectory(val);
      }
      src = std::dynamic_pointer_cast<TileSourceWebMapService>(body.second->getIMGtileSources()[0]);
      if (src) {
        src->setCacheDirectory(val);
      }
    }
